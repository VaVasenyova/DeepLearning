...
let state = {
  ...
  baselineModel: null,
  studentModel: null,
  baselineOptimizer: null, // Separate optimizer for baseline
  studentOptimizer: null,  // Separate optimizer for student
};
...
// Implemented 'transformation' and 'expansion' architectures.
// ------------------------------------------------------------------
function createStudentModel(archType) {
  ...
  if (archType === "compression") {
    ...
    // [FIXED] Transformation: 1:1 mapping (maintain dimension)
    // Hidden layer size equal to input size (256 = 16*16)
    model.add(tf.layers.dense({ units: 256, activation: "relu" }));
    model.add(tf.layers.dense({ units: 256, activation: "sigmoid" }));
  } else if (archType === "expansion") {
    // [FIXED] Expansion: Overcomplete (increase dimension)
    // Hidden layer larger than input size (512 > 256)
...
// Modified to create a smooth gradient using Smoothness + Direction.
// Minimized MSE to avoid identity mapping (copying).
// ------------------------------------------------------------------
function studentLoss(yTrue, yPred) {
  return tf.tidy(() => {
    // 1. Basic Reconstruction (MSE) - VERY LOW WEIGHT
    // We keep minimal MSE to preserve colors, but not positions
    const lossMSE = mse(yTrue, yPred).mul(0.01);

    // 2. Smoothness - "Be smooth locally" (remove noise)
    // Higher weight to encourage smooth gradients
    const lossSmooth = smoothness(yPred).mul(0.5);

    // 3. Direction - "Be bright on the right" (create gradient)
    // Medium weight to guide the direction
...
async function trainStep() {
  ...
  // Using separate optimizer to avoid architecture conflict
  const baselineLossVal = tf.tidy(() => {
    ...
    state.baselineOptimizer.applyGradients(grads); // Use baseline optimizer
  ...
  try {
    studentLossVal = tf.tidy(() => {
      ...
      state.studentOptimizer.applyGradients(grads); // Use student optimizer
      return value.dataSync()[0];
    });
...
function resetModels(archType = null) {
  ...
  // Important: Dispose BOTH optimizers because they hold references to old model variables.
  ...
  // Create NEW SEPARATE optimizers (must be done AFTER models are created)
